...
<div class="row">
    ...
    <div class="col-md-12 mt" style="height: 590px;overflow: auto;" >
        <h4><i class="fa fa-angle-right">销售退货单</i></h4>
        <div class="col-sm-12 search-collapse">
            <p class="select-title"></p>
            <div class="select-list">
                <ul>
                    <li id="selectCustomer">
        ...
        <div class="content-panel" style="height: 395px;overflow: auto;" >
            <table id="addSalesBackOrder"
                   data-toggle="table"
            ...
            <div class="form-group" style="padding-bottom: 10px;">
                ...
                <!--退货理由-->
                <div>
                    <textarea class="form-control"  name="sboReason" id="sboReason" rows="2" placeholder="请填写退货原因" rows="4" data-rule="required" data-msg="退货原因"></textarea>
                </div>
            </div>
        </div>
...
<div  class="modal fade bs-example-modal-lg" id="phoneModal" tabindex="-1" role="dialog" data-backdrop="static" data-keyboard="false" aria-labelledby="phoneModalLabel">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="content-panel rowSameHeight"  >
...
<script>
            ...
            success:function(data){
                if(data.code==0){
                    ...
                    $.each(warehouseList,function(i,item){
                        if (item.warehouseName=="退货仓库"){
                            $("#warehouseId").append("<option value='"+item.warehouseId+"' selected>"+item.warehouseName+"</option>");
                        }else{
     ...
     * 实现表格可编辑
     */
    $('#addSalesBackOrder').bootstrapTable({
    ...
    function getAllProducts() {
        ...
        $('#products').bootstrapTable({
            url: "/business/serialNumber/getSalesPhoneInfo",//数据源
            dataField: "salesPhoneInfo",//服务端返回数据键值 就是说记录放的键值是rows，分页时使用总记录数的键值为total
            //height: 300,//高度调整
    ...
    function refreshTable() {
        $('#products').bootstrapTable('refresh', {
            url: "//business/serialNumber/getSalesPhoneInfo",
            pageSize: 10,
            pageNumber: 1
        });
    ...
    function saveData(index, field, value) {
        $("#addSalesBackOrder").bootstrapTable('updateCell', {
            index: index,       //行索引
            field: field,       //列名
            value: value        //cell值
        })
    ...
    function getSelectedProduct(){
        ...
        for (var i=0;i<info.length;i++){
            ...
            var id=info[i].id;
            var sn=info[i].sn;
            var unitPrice=info[i].salesDetail.unitPrice;

            $('#addSalesBackOrder').bootstrapTable('insertRow', {
                index: $('#addSalesBackOrder').bootstrapTable('getOptions').totalRows,
                row: {
                    phoneId: phoneId,
                    phoneName: phoneName,
        ...
        $('#addSalesBackOrder').bootstrapTable({height:$(window).height()-120});
    ...
    function submitOrder() {
        ...
        var salesBackDetails=JSON.stringify($('#addSalesBackOrder').bootstrapTable('getData'));
        ...
        //alert(customerId);
        //alert(salesBackDetails)

        //return;

        $.ajax({
            url:'/business/salesBackOrder/addSalesBackOrder',
            data:{salesBackDetails:salesBackDetails,customerId:customerId,warehouseId:warehouseId,sboReason:sboReason},
            dataType:'json',
            type:'post',
            success:function(data){
                if(data.code==0){
